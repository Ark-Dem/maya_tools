...
#
# 2024.01.26 (v2.2) - update for Python3
#
...
# UPDATE PYTHON3 RENAMED FUNCTIONS TO KEEP THEM WORKING WITH PYTHON2
try:
	xrange(1)
except:
	xrange = range
				...
				if pid not in pids[instList.index(inst)]:
					...
					if not objExists( instGrp ):
						...
						if dupOrInst == 2:
							...
							origShapes = ls( listRelatives( dup, shapes = True, fullPath = True ), intermediateObjects = True )
							if origShapes != []:
								delete( origShapes )
						parent( dup, instGrp )
